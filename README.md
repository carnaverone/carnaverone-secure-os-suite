...
```bash
chmod +x setup_full_ubuntu.sh setup_full_arch.sh setup_full_popos.sh setup_full_debian.sh setup_full_fedora.sh setup_full_centos.sh setup_full_macos.sh setup_full_windows_wsl.sh setup_pop_os_prog_intall_bonus.sh
```
...
```bash
./setup_pop_os_prog_intall_bonus.sh
```
...
⚙️ What Does This Suite Install? 
⚙️ Qu'installe cette suite ?
---
...
UFW (Uncomplicated Firewall) — Easy-to-use firewall configuration. 
                               Configuration de pare-feu simple d'utilisation.

Fail2Ban — Intrusion prevention system to block IPs after failed login attempts. 
           Système de prévention des intrusions pour bloquer les IP après des tentatives de connexion échouées.
...
Systemd Services — Manages and runs system services for automation. 
                   Gère et exécute des services système pour l'automatisation.
