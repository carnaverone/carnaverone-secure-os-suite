...
sudo apt install -y build-essential curl wget git vim nano unzip zip htop neofetch net-tools software-properties-common python3-pip
...
sudo apt install -y qemu-kvm libvirt-daemon-system libvirt-clients bridge-utils virtualbox virtualbox-ext-pack virt-manager
...
ExecStart=/usr/bin/python3 $USER_HOME/tts_server.py
Restart=always
